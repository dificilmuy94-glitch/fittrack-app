import { Chrome as Home, CalendarDays, TrendingUp, FolderOpen } from 'lucide-react';
        ...
        {navItems.map(({ id, label, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={cn(
                'flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-all duration-200 min-w-[56px] touch-target',
            ...
            >
              <div className={cn('p-1.5 rounded-lg transition-all duration-200', isActive ? 'bg-[#1A3A32]/10 dark:bg-emerald-400/10' : '')}>
                <Icon size={20} strokeWidth={isActive ? 2.5 : 1.8} className={cn('transition-all duration-200', isActive ? 'scale-110' : '')} />
              </div>
              <span className={cn('text-[10px] font-medium tracking-tight transition-all duration-200', isActive ? 'opacity-100' : 'opacity-60')}>
                {label}
              </span>
            </button>
          );
